...
SELECT artist.name, COUNT(song.name) AS number_of_songs FROM song
...
SELECT artist.name, COUNT(song.id) AS number_of_songs FROM song
INNER JOIN artist ON artist.id = song.artist_id
...
SELECT artist.name as artist_name, COUNT(song.id) AS num_of_songs FROM artist
